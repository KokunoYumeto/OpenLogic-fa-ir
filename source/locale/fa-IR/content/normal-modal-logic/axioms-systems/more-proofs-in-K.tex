...
\olsection{اثبات‌های بیشتر در~\Log{K}}

اکنون چند نمونهٔ دیگر از !!{derivability} در~\Log{K} را، این بار با
استفاده از روش ساده‌شدهٔ معرفی‌شده در~\olref[der]{sec}، می‌بینیم.
...
5. &  $\Log{K} \Proves \Box(!A \lif !B) \lif (\Diamond!A \lif \Diamond!B)$ & $\Diamond$ به‌جای~$\lnot\Box\lnot$.
\end{derivation}
\end{proof}
...
\begin{proof}
  ...
  \Diamond !B)$ & $\Diamond$ به‌جای~$\lnot\Box\lnot$.
  \end{derivation}
\end{proof}
...
\begin{proof}
  \begin{derivation}
    ...
    $\Diamond$ به‌جای~$\lnot\Box\lnot$\\
    5. & $\Log{K} \Proves \Diamond!B \lif \Diamond(!A \lor !B)$ & به‌طور مشابه\\
...
\begin{proof}
  \begin{derivation}
    ...
    \Diamond!B \lif \Diamond!A)$ & $\Diamond$ به‌جای~$\lnot\Box\lnot$\\
    7. & $\Log{K} \Proves \Diamond(!A\lor!B) \lif (\Diamond!A \lor \Diamond!B)$ & \PL, 6. \\
  \end{derivation}
\end{proof}

\begin{prob}
  نشان دهید ادعاهای !!{derivability} زیر برقرارند:
  \begin{enumerate}
