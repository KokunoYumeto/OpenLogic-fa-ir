...
\olsection{اثبات‌ها در~\Log{K}}

برای تمرین اثبات در کوچک‌ترین دستگاه موجهاتی، نشان می‌دهیم همهٔ
!!{formula}های معتبر در سمت چپِ
\olref[syn][sch]{tab:valid-invalidSchemas} اثبات‌هایی در~\Log{K} دارند.
...
\end{derivation}
توجه کنید !!{formula} خط~$9$ نمونه‌ای از همان‌گویی زیر است:
\[
...
\begin{proof}
  ...
  \end{derivation}
  !!{formula}های خطوط~$6$ و~$9$ نمونه‌هایی از همان‌گویی‌های زیرند:
  \begin{align*}
...
\begin{proof}
  ...
  \end{derivation}
  !!{formula}های خطوط~$8$ و~$10$ نمونه‌هایی از همان‌گویی‌های زیرند:
  \begin{align*}
  ...
  \end{derivation}
  خط آخر~$\lnot \Box p \lif \Diamond\lnot p$ است، زیرا~$\Diamond$
  به‌صورت~$\lnot\Box\lnot$ تعریف شده است. !!{formula} خط~$5$ نمونه‌ای
  از همان‌گویی زیر است:
  \[
    (p \lif q) \lif (\lnot q \lif \lnot p).
  \]
  }}{% Box is defined
  ...
  \end{derivation}
  !!{formula} خط~$4$ نمونه‌ای از همان‌گویی زیر است:
  \begin{align*}
...
\begin{prob}
  !!{derivation}هایی در~$\Log{K}$ برای !!{formula}های زیر بیابید:
  \begin{enumerate}
