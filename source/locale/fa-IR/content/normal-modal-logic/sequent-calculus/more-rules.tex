...
\begin{table}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{قواعد سکوئنت برای منطق‌های موجهاتی گوناگون.}
  \ollabel{tab:logics-rules}
\end{table}

\iftag{prvBox}{
\begin{ex}
  یک !!{derivation} سکوئنت ارائه می‌کنیم که نشان می‌دهد
  $\Log{K4} \Proves \Ax{4}$؛ یعنی~$\Box!A \lif \Box\Box!A$.
  \begin{prooftree}
...
\begin{ex}
  یک !!{derivation} سکوئنت ارائه می‌کنیم که نشان می‌دهد
  $\Log{K4} \Proves \Ax{4}$؛ یعنی~$\Diamond\Diamond!A \lif \Diamond!A$.
  \begin{prooftree}
...
\begin{ex}
حساب سکوئنت برای~\Log{S5} بدون قاعدهٔ~\Cut{} کامل نیست؛ برای نمونه،
\iftag{prvBox}{$\Diamond\Box !A \lif !A$}{$!A \lif
\Box\Diamond !A$}، که در~$\Log{S5}$ معتبر است، بدون~\Cut{} اثباتی
ندارد. این یک !!a{derivation} با استفاده از~\Cut{} است:
\begin{prooftree}
...
\begin{prob}
!!{derivation}های سکوئنتی ارائه کنید که موارد زیر را نشان دهند:
  \begin{enumerate}
