...
\DisplayProof
\end{tabular}
\caption{قواعد \Log{G3i} برای منطق شهودی. در قواعد
$\RightR{\lforall}$ و$\LeftR{\lexists}$، !!{constant}~$a$ نباید در
دنبالهٔ نتیجه رخ دهد. $\Gamma$ چندمجموعه‌ای از !!{formula}s است و
$\Delta$ می‌تواند تهی باشد یا یک !!{formula} داشته باشد. در اصول،
!!{formula}~$!C$ باید اتمی باشد. دستگاه~\Log{G3m} همان~\Log{G3i}
بدون اصول $\lfalse,\Gamma\Sequent\Delta$ است.}
\ollabel{tab:G3i}
\end{table}
